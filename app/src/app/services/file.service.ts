...
export class FileService extends ApiService {
  ...
  uploadFile(file: File): Observable<FileModel> {
    ...
    return this.post<FileModel>('/files', file);
  }

  // GET /files/{id} - Get file by ID (not in swagger but likely needed)
  getFileById(fileId: number): Observable<FileModel> {
    if (environment.useMockData){// Mock data with specific files for development
    ...
    return this.get<FileModel>(`/files/${fileId}`);
  }
  ...
  downloadFile(fileId: number): Observable<Blob> {
    ...
    if (environment.useMockData){// Get file info first to determine content type
    ...
    return this.get<Blob>(`/files/${fileId}/download`);
  }
